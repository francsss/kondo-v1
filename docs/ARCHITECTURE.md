...
- `src/lib/notifications.ts` owns safe internal links, templates, preferences, outbox enqueueing, deduplication, bounded retries, stale-lock recovery, member read/hide DTOs, persisted City/Community/University announcement audiences, and Admin diagnostics. Completed jobs optionally fan out through `src/lib/push-notifications.ts`; VAPID credentials and subscription endpoints stay server-side, expired subscriptions are disabled, and Push failures never roll back an already persisted in-app notification.
