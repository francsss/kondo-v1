...
- `src/lib/media.ts` is the media lifecycle boundary. It owns signed upload authorization, server-generated object keys, ownership, validation activation, replacement, attachment identity, secure delivery authorization, soft removal, Admin inspection, audit, and orphan cleanup. `src/lib/storage.ts` keeps local and S3-compatible drivers behind one contract.
- `src/lib/profiles.ts` is the profile and account-request boundary. It owns stable public/member/owner DTOs, field-level audience rules, coherent visible counters, profile editing, validated avatar attachment, saved-content resolution, profile reports, data-export/deletion requests, and safe Admin user review.
...
- `src/lib/notifications.ts` owns safe internal links, templates, preferences, outbox enqueueing, deduplication, bounded retries, stale-lock recovery, member read/hide DTOs, announcements, and Admin diagnostics.
...
- City exploration: typed city and section contracts, a slug-based city registry, reusable city/section routes, and editorial source links. Jiaxing is the first registered city; adding another city does not require a new page or component architecture.
...
- Search is isolated behind a query function and `/api/search`; PostgreSQL text search can be replaced by Typesense, Meilisearch, or OpenSearch without changing page contracts.
...
- The in-process rate limiter is an MVP guard. Multi-instance production should use a shared Redis-compatible store.
