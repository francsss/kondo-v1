...
| `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` | Mandatory public | Google Maps JavaScript rendering and public study-area geocoding for privacy-safe Meet discovery | Google Cloud Console → enable Maps JavaScript API and Geocoding API → create a browser key restricted to Kondo Production and approved Preview HTTP referrers. |
