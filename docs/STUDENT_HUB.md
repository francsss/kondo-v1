...
3. The server-only `ScheduleAnalysisProvider` reads the private objects and sends them to the configured provider. The browser never receives `OPENAI_API_KEY`.
4. The OpenAI Responses request uses strict structured output and `store: false`. It supports English and Chinese timetable notation such as `第1-2节`, `1-16周`, `单周`, and `双周`.
...
- `OPENAI_API_KEY` — required in Vercel Production, server-only.
- `SCHEDULE_AI_PROVIDER` — optional; defaults to `openai`.
- `SCHEDULE_AI_MODEL` — optional reviewed model override.
- `SCHEDULE_AI_TIMEOUT_MS` — optional 10–120 second timeout; default 60 seconds.
