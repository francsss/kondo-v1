-- Repair: load the guide content pack on databases that skipped it.
--
-- 20260816150000_guide_content_pack resolves its author with
--
--   SELECT id FROM "User" WHERE role = 'SUPER_ADMIN' ORDER BY "createdAt" LIMIT 1
--
-- and returns early when that finds nothing. On a fresh database migrations run
-- before seeds, so there is no SUPER_ADMIN yet: the entire library is skipped,
-- the migration is recorded as applied, and it never runs again. Reproduced
-- exactly that way locally — the database reported zero guides until the SQL
-- was replayed by hand after a SUPER_ADMIN existed.
--
-- This replays the same inserts with a resolution that can actually succeed:
-- the earliest SUPER_ADMIN, else the earliest ADMIN, else the earliest user of
-- any role. Authorship here is a foreign key on a row of reference content, not
-- a claim about who wrote it; the pack's provenance lives in its sources and in
-- its NEEDS_REVIEW status, neither of which this touches.
--
-- Every insert is still guarded on slug, so on a database where the original
-- migration did run this changes nothing at all. Guides an editor has since
-- edited, verified, or archived are left exactly as they are.
...
BEGIN
  ...
  IF author_id IS NULL THEN
    SELECT "id" INTO author_id FROM "User"
     WHERE "role" = 'ADMIN' ORDER BY "createdAt" ASC LIMIT 1;
  END IF;
  IF author_id IS NULL THEN
    SELECT "id" INTO author_id FROM "User" ORDER BY "createdAt" ASC LIMIT 1;
  END IF;
  IF author_id IS NULL THEN
    RAISE NOTICE 'No users at all; guide content pack still pending.';
    RETURN;
  END IF;
  ...
  -- Superseded slugs are archived, never deleted: deletion cascades through
  -- GuideStep to GuideProgress and would destroy students' checklist progress.
  UPDATE "Guide" SET "contentStatus" = 'ARCHIVED'::"GuideContentStatus", "published" = false
   WHERE "slug" IN ('residence-permit-without-the-panic', 'open-a-chinese-bank-account', 'first-72-hours-in-china')
     AND "contentStatus" <> 'ARCHIVED'::"GuideContentStatus";
END $$;
