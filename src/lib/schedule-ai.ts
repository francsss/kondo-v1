...
class DeepSeekScheduleProvider implements ScheduleAnalysisProvider {
  async analyze(
    ...
    const systemPrompt = [
      ...
      "Read Chinese, English, and mixed Chinese-English timetable text.",
      "Extract only: weekday, start period, end period, course name, classroom, and teacher.",
      "Ignore decorative layout, colors, merged-cell styling, semester metadata, week ranges, and other nonessential information.",
      "Prefer numbered periods. When the document only has clock times, return startTime and endTime as HH:MM and leave periods null.",
      "Do not invent missing values. Teacher and room are optional and may be null.",
      "A valid row needs a weekday, course name, and either a period range or a clock-time range.",
      "Keep partially readable courses and lower confidence instead of rejecting the entire document.",
    ...
    const userPrompt = [
      ...
      `Official period mapping: ${JSON.stringify(context.periods)}. Use it to identify period numbers from row positions or printed times, but never invent a period that cannot be supported by the document.`,
