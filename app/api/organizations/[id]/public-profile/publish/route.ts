...
import { publishOrganizationPublicProfile } from "@/lib/organization-publication";
...
export async function POST(
  ...
  if (!parsed.success) return jsonError("Confirm publication to continue.");
  try {
    return Response.json(
      await publishOrganizationPublicProfile(
        user,
        (await params).id,
    ...
    return organizationApiFailure(
      "organizations.public-profile.publish",
      error,
    );
  }
}
