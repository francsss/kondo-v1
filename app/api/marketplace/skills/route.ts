...
export async function POST(request: NextRequest) {
  ...
  if (
    !(await rateLimit(`skill-offer:${user.id}`, 8, 24 * 60 * 60_000)).allowed
  ) {
    return jsonError("Daily skill offer limit reached.", 429);
  }
  ...
  try {
    ...
    const offer = await prisma.studentSkillOffer.create({
      data: {
        ...parsed.data,
        ownerId: user.id,
    ...
    return internalApiError("marketplace.skills.create", error);
  }
}
