...
import { createOrReuseListingReport, MarketplaceError } from "@/lib/marketplace";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await rateLimit(`listing-report:${user.id}`, 12, 24 * 60 * 60_000)).allowed) return jsonError("Report limit reached.", 429);
  ...
  try {
    return Response.json(await createOrReuseListingReport({
      actor: user,
