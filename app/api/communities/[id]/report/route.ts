...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await rateLimit(`content-report:${user.id}`, 12, 24 * 60 * 60_000)).allowed) return jsonError("Report limit reached.", 429);
  ...
  try {
    return Response.json(await createOrReuseCommunityContentReport({ actor: user, targetType: "Community", targetId: (await params).id, ...parsed.data, meta: getRequestMeta(request) }), { status: 201 });
  } catch (error) {
    if (error instanceof CommunityError) return jsonError(error.message, error.status);
    return internalApiError("communities.report", error);
  }
}
