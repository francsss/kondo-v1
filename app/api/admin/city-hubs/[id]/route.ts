...
export async function GET(_: NextRequest, { params }: Context) {
  ...
  try {
    ...
    if (!hub) return adminJson({ error: "City hub not found." }, { status: 404 });
...
export async function PATCH(request: NextRequest, { params }: Context) {
  ...
  if (!auth.authorized) return auth.error;
  const parsed = cityHubUpdateSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return adminJson({ error: parsed.error.issues[0]?.message ?? "Invalid city hub content." }, { status: 400 });
  }
  try {
    return adminJson(
