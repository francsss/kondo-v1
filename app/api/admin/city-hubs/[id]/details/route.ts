...
import { CityHubError, updateCityHubDetails } from "@/lib/city-hub";
import { getRequestMeta, hasTrustedOrigin } from "@/lib/request";
import { cityHubDetailsUpdateSchema } from "@/lib/validation";
...
export async function PATCH(request: NextRequest, { params }: Context) {
  ...
  const parsed = cityHubDetailsUpdateSchema.safeParse(
    await request.json().catch(() => null),
  );
  if (!parsed.success) {
    return adminJson(
      { error: parsed.error.issues[0]?.message ?? "Invalid hub details." },
      { status: 400 },
    );
  }
  try {
    ...
    return adminInternalError("admin.city-hubs.details.update", error);
  }
}
