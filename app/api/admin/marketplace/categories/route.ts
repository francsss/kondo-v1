...
import { listMarketplaceCategories, MarketplaceError, upsertMarketplaceCategory } from "@/lib/marketplace";
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) return adminJson({ error: parsed.error.issues[0]?.message ?? "Invalid category." }, { status: 400 });
  try {
    return adminJson(await upsertMarketplaceCategory({ actor: auth.user, data: parsed.data, meta: getRequestMeta(request) }), { status: 201 });
  } catch (error) {
