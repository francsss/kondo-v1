...
import { deleteMarketplaceCategory, MarketplaceError, upsertMarketplaceCategory } from "@/lib/marketplace";
...
export async function PATCH(request: NextRequest, { params }: Context) {
  ...
  const parsed = marketplaceCategorySchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return adminJson({ error: "Invalid category." }, { status: 400 });
  try {
    return adminJson(await upsertMarketplaceCategory({ actor: auth.user, categoryId: (await params).id, data: parsed.data, meta: getRequestMeta(request) }));
  } catch (error) {
...
export async function DELETE(request: NextRequest, { params }: Context) {
  ...
  if (!auth.authorized) return auth.error;
  try {
    await deleteMarketplaceCategory({ actor: auth.user, categoryId: (await params).id, meta: getRequestMeta(request) });
