...
import { getAdminListing, MarketplaceError, updateListingAsAdmin } from "@/lib/marketplace";
...
export async function GET(_: NextRequest, { params }: Context) {
  ...
  try {
    ...
    if (!listing) return adminJson({ error: "Listing not found." }, { status: 404 });
...
export async function PATCH(request: NextRequest, { params }: Context) {
  ...
  if (!auth.authorized) return auth.error;
  const parsed = adminListingUpdateSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return adminJson({ error: "Invalid listing update." }, { status: 400 });
  try {
    return adminJson(await updateListingAsAdmin({
      actor: auth.user,
