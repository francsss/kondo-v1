...
export async function GET(_: NextRequest, { params }: Context) {
  ...
  try {
    ...
    if (!community)
      return adminJson({ error: "Community not found." }, { status: 404 });
...
export async function PATCH(request: NextRequest, { params }: Context) {
  ...
  if (!auth.authorized) return auth.error;
  const parsed = adminCommunityUpdateSchema.safeParse(
    await request.json().catch(() => null),
  );
  if (!parsed.success)
    return adminJson({ error: "Invalid community update." }, { status: 400 });
  try {
    return adminJson(
