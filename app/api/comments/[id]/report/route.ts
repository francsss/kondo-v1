...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return Response.json(await createOrReuseCommunityContentReport({ actor: user, targetType: "Comment", targetId: (await params).id, ...parsed.data, meta: getRequestMeta(request) }), { status: 201 });
  } catch (error) {
    if (error instanceof CommunityError) return jsonError(error.message, error.status);
    return internalApiError("comments.report", error);
  }
}
