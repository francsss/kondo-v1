...
export default async function AdminListingPage({
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 pb-28 pt-7 sm:px-6 lg:px-8 lg:pb-16 lg:pt-10">
      <PageHeader
        description={`${listing.category.name} · ${listing.city.name} · seller ${listing.seller.fullName}`}
        eyebrow="Marketplace CMS"
        title={listing.title}
      />
      <AdminNav
        currentPath={`/admin/marketplace/${listing.id}`}
        role={user.role}
      />
      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
        <div className="space-y-6">
          <MarketplaceAdminActions
            initialReviewed={Boolean(listing.fraudReviewedAt)}
          ...
          <Card>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {listing.fraudFlags.map((flag) => (
                <span
                  className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-black text-amber-700"
                  key={flag}
                >
                  {flag}
                </span>
        ...
        <div className="space-y-6">
          <Card>
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black dark:bg-white/10">
                {listing.status}
              </span>
            </div>
