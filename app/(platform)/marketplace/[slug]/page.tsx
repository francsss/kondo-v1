  ...
  return (
    <div className="mx-auto max-w-[1120px] px-4 pb-28 pt-7 sm:px-6 lg:px-8 lg:pb-16">
      ...
      <div className="mt-4 grid gap-7 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
        <div
          className={`grid gap-3 ${listing.images.length > 1 ? "sm:grid-cols-2" : ""}`}
        >
          {listing.images.length ? (
            listing.images.map((image, index) =>
              image.mediaId ? (
                <MediaImage
                  ...
                  className={`h-full w-full rounded-4xl object-cover shadow-soft ${index === 0 && listing.images.length > 2 ? "sm:col-span-2 aspect-[16/9]" : "aspect-[4/3]"}`}
                  height={900}
                  key={image.id}
            ...
            <div className="grid aspect-[4/3] place-items-center rounded-4xl bg-kondo-mint text-9xl">
        ...
        <div>
          ...
          <h1 className="mt-5 text-3xl font-black tracking-[-0.045em] text-kondo-ink dark:text-white sm:text-4xl">
            {listing.title}
          </h1>
          ...
          {owner ? (
            <Button asChild className="mt-5 w-full" variant="secondary">
              <Link href={`/marketplace/${listing.slug}/edit`}>
                <Edit3 className="h-4 w-4" /> Edit your listing
              </Link>
            </Button>
          ) : (
            <MessageUserButton
